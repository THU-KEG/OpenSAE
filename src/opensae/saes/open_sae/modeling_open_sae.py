...
class OpenSae(PreTrainedOpenSae):
    def __init__(
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            print(f"\n[Global Init Check] Full Encoder Head: {[round(x, 6) for x in head_values]}")
            print(f"[Global Init Check] Full Encoder Sum: {total_sum:.6f}\n")
    ...
    def _compute_moe_losses(self, router_logits: Tensor):
        ...
        if dist.is_initialized():
            ...
            dist.all_reduce(stats_reduced, op=dist.ReduceOp.SUM) 
            stats = stats + stats_reduced.detach() - stats.detach()
    ...
    def encode(self, hidden: Tensor, expert_scale: Tensor | None = None, return_all_features: bool = False, selected_experts: Tensor | None = None) -> SaeEncoderOutput:
        sae_input, input_mean, input_std = self.pre_process(hidden)
        
        if not self.training and self.use_moe and selected_experts is not None and getattr(self.config, 'k_experts', 1) == 1:
            batch_size = sae_input.shape[0]
            # 这里的 all_features 是 pre-acts
            all_features = torch.zeros(batch_size, self.local_feature_size, device=sae_input.device, dtype=self.encoder.weight.dtype)
            
            for global_expert_id in range(self.config.num_experts):
                if not (self.my_expert_start_idx <= global_expert_id < self.my_expert_end_idx):
                    continue 
                
                local_expert_id = global_expert_id - self.my_expert_start_idx
                token_mask = (selected_experts.flatten() == global_expert_id)
                if not token_mask.any():
                    continue 
                
                expert_inputs = sae_input[token_mask]
                start_dim = local_expert_id * self.global_features_per_expert
                end_dim = (local_expert_id + 1) * self.global_features_per_expert
                
                expert_weight = self.encoder.weight[start_dim:end_dim, :]
                expert_bias = self.encoder.bias[start_dim:end_dim]
                
                expert_out = torch.nn.functional.linear(expert_inputs, expert_weight, expert_bias)
                all_features[token_mask, start_dim:end_dim] = expert_out
        else:
            # ✅ 修复 1：提取未经过 ReLU 的 pre_acts
            pre_acts = self.encoder(sae_input)
            if expert_scale is not None:
                all_features = pre_acts * expert_scale
            else:
                all_features = pre_acts
                
        # ✅ 主干网络依然需要 ReLU 进行 TopK 选择
        all_features_post_relu = torch.nn.functional.relu(all_features)
        feature_activation, feature_indices = self.sparse_activation(all_features_post_relu)
        
        return SaeEncoderOutput(
            sparse_feature_activations = feature_activation,
            sparse_feature_indices = feature_indices,
            # 🚨 极其关键：将 pre-acts 传输出去，供后续 AuxK 计算梯度！
    ...
    def auxk_loss(
        ...
        hidden_variance: Tensor, 
        dead_mask: Tensor,
        pre_acts: Tensor, # ✅ 接收 pre_acts
        input_mean: Tensor | None = None,
        input_std: Tensor | None = None,
        expert_mask: Tensor | None = None # ✅ 接收 expert_mask
    ) -> Tensor:
        ...
        # ✅ 修复 2：只允许从 “当前被选中的专家”的死神经元里寻找，防止梯度再次被 expert_scale 切断
        valid_mask = dead_mask[None, :].expand(pre_acts.shape[0], -1)
        if expert_mask is not None:
            valid_mask = valid_mask & expert_mask

        # 屏蔽掉非目标神经元
        auxk_pre_acts = torch.where(valid_mask, pre_acts, -torch.inf)
        local_vals, local_inds = auxk_pre_acts.topk(k_aux_limit, sorted=False)
        ...
        # =======================================================
        # ✅ 防作弊修复：位置移到这里，确保各种并行模式下都能生效
        # =======================================================
        auxk_feature_activations = torch.nn.functional.relu(auxk_feature_activations)
        ...
        # ✅ 数学公式修复：剥离偏置项
        w_dead_a_dead = auxk_sae_decoder_output - self.b_dec
        
        # 拟合残差误差
        auxk_loss = (w_dead_a_dead + reconstruction_error).pow(2).sum(0)
    ...
    def forward(
        ...
        # 判断是否需要返回 all_features（pre-acts）给 AuxK 或 Multi-TopK 用
        needs_all_features = self.config.multi_topk or (self.config.auxk_alpha > 0 and dead_mask is not None)
        
        sae_enc_out = self.encode(
            hidden, 
            expert_scale=expert_scale, 
            return_all_features=needs_all_features,
        ...
        if dead_mask is not None:
            # ✅ 传入 pre_acts 和 expert_mask
            auxk_loss = self.auxk_loss(
                hidden, sae_dec_out, recon_err, var, dead_mask, 
                sae_enc_out.all_features, 
                sae_enc_out.input_mean, sae_enc_out.input_std,
                expert_mask=expert_mask_for_logging
            )
        else:
            auxk_loss = torch.tensor(0.0, device=hidden.device)

        if self.config.multi_topk:
            # ✅ 修复 3：因为 sae_enc_out.all_features 是 pre-acts，所以算 multi-topk 前要套一个 ReLU
            post_relu_all_features = torch.nn.functional.relu(sae_enc_out.all_features)
            multi_topk_feature_activations, multi_topk_feature_indices = self.multi_topk(post_relu_all_features)
            ...
            multi_topk_loss = sae_dec_out.new_tensor(0.0)
        
        if getattr(self.config, "l1_coef", 0) > 0:
            # ✅ 同理，L1 惩罚也必须作用在激活后的张量上
            post_relu_all_features = torch.nn.functional.relu(sae_enc_out.all_features)
            l1_loss = torch.norm(post_relu_all_features, p=1, dim=-1).mean() * self.config.l1_coef
        else:
            l1_loss = torch.tensor(0.0, device=hidden.device)

        final_loss = recon_loss + auxk_loss * self.config.auxk_alpha + moe_cv_loss + router_z_loss + l1_loss + multi_topk_loss/8
        ...
        return SaeForwardOutput(
            ...
            expert_mask = expert_mask_for_logging,
            auxk_loss = auxk_loss # ✅ 保证你的 Trainer 可以成功 log 出来
        )