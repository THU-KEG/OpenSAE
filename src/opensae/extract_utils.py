...
def refine_tokens(token_ids: List[str], tokenizer: AutoTokenizer) -> dict:
    decoded_tokens = [tokenizer.decode(token_ids[:i+1]) for i in range(len(token_ids))]
...
def refine_tokens_in_sentence(sentence: str, tokenizer: AutoTokenizer) -> dict:
