...
def flash_attention_forward(
    ...
    cu_seqlens = None, max_seqlens = None,
...
def llama_attention(
    ...
    max_seqlens: Optional[torch.LongTensor] = None,
        **kwargs,
        ...
        attn_output, attn_weights = self._flash_attention_forward_2(
            query_states,
            key_states,
            value_states,
            attention_mask,
...
def model_patch(model):
    ...
    for l in model.layers:
        ...
        l.self_attn.forward = types.MethodType(llama_attention, l.self_attn)
        l.self_attn._flash_attention_forward_2 = types.MethodType(flash_attention_forward, l.self_attn)


    return model

if __name__ == "__main__":
    tokenizer = AutoTokenizer.from_pretrained("/data0/hjw/trainsae/Qwen3-1.7B", use_fast = True)
...
ARG VERSION
    ...
    for t in text:
        ...
        print(tt)
        tt = [tokenizer.eos_token_id,] + tt + [tokenizer.eos_token_id]
        print(tt)
        input_ids_list.extend(tt)
        seperate_input_ids.append(tt)
        seq_lengths_list.append(len(tt)) # 记录长度

    input_ids = torch.tensor(input_ids_list)
    
    ####### What collator will do #########
    cu_seqlens = torch.tensor([0] + seq_lengths_list, dtype=torch.int32).cumsum(0)
    
    seq_lens = cu_seqlens[1:] - cu_seqlens[:-1]
    max_seq_lens = torch.max(seq_lens)
    
    print("Correct cu_seqlens:", cu_seqlens)

    model = AutoModel.from_pretrained(
        "/data0/hjw/trainsae/Qwen3-1.7B", 
        torch_dtype=torch.bfloat16).to("cuda:2").eval()
    model = model_patch(model)
    
    cu_seqlens = cu_seqlens.to("cuda:2").to(torch.int32)
    input_ids = input_ids.to("cuda:2").unsqueeze(0)
    max_seq_lens = max_seq_lens.to("cuda:2").to(torch.int32)
    
    varlen_model_output = model(input_ids, cu_seqlens = cu_seqlens, max_seqlens = max_seq_lens)
    no_varlen_model_output = model(input_ids)
    
    seperate_input_ids = [torch.tensor(x).to("cuda:2").unsqueeze(0) for x in seperate_input_ids]
